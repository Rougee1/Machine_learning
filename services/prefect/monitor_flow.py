...
def decide_action(as_of_ref: str, as_of_cur: str, drift_share: float, target_drift: float, threshold: float = 0.02) -> str:
    """
    Décision : si drift_share dépasse threshold, on déclenche un retrain réel via train_and_compare_flow.
    """
...
if __name__ == "__main__":
    # Exécution en mode standalone sans serveur Prefect si PREFECT_API_DISABLED=true
    if os.getenv("PREFECT_API_DISABLED") == "true":
        # Mode standalone : on exécute directement les fonctions sans Prefect
        # On appelle les fonctions directement (pas via les tasks Prefect)
        ref_df = get_final_features(AS_OF_REF_DEFAULT)
        cur_df = get_final_features(AS_OF_CUR_DEFAULT)
        # Appeler la fonction directement (pas la task)
        if "churn_label" not in ref_df.columns or "churn_label" not in cur_df.columns:
            print("[Target drift] churn_label absent -> target drift non calculé")
            tdrift = float("nan")
        else:
            if ref_df["churn_label"].dropna().empty or cur_df["churn_label"].dropna().empty:
                print("[Target drift] labels vides -> target drift non calculé")
                tdrift = float("nan")
            else:
                ref_rate = float(ref_df["churn_label"].astype(int).mean())
                cur_rate = float(cur_df["churn_label"].astype(int).mean())
                tdrift = abs(cur_rate - ref_rate)
                print(f"[Target drift] ref_rate={ref_rate:.4f} cur_rate={cur_rate:.4f} abs_diff={tdrift:.4f}")
        
        # Appeler run_evidently directement
        Path(REPORT_DIR).mkdir(parents=True, exist_ok=True)
        DRIFT_SHARE_THRESHOLD = 0.5
        metrics = [
            DataSummaryPreset(),
        ...
        report = Report(metrics=metrics)
        eval_result = report.run(
            reference_data=build_dataset_from_df(ref_df),
            current_data=build_dataset_from_df(cur_df),
        )
        html_path = Path(REPORT_DIR) / f"drift_{AS_OF_REF_DEFAULT}_vs_{AS_OF_CUR_DEFAULT}.html"
        json_path = Path(REPORT_DIR) / f"drift_{AS_OF_REF_DEFAULT}_vs_{AS_OF_CUR_DEFAULT}.json"
        eval_result.save_html(str(html_path))
        eval_result.save_json(str(json_path))
        summary = eval_result.dict()
        pprint(summary)
        ...
        # Appeler decide_action directement
        threshold = 0.3
        if drift_share >= threshold:
            msg = (
                f"RETRAINING_TRIGGERED (SIMULÉ) drift_share={drift_share:.2f} >= {threshold:.2f} "
                f"(target_drift={tdrift if tdrift == tdrift else 'NaN'})"
            )
        else:
            msg = (
        ...
        # Si on a une URL Prefect configurée, on utilise flow.run()
        monitor_month_flow()
